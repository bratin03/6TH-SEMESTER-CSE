...
class NeuralNetwork:
    ...
    def dataLoader(self):
        ...
        self.test_data = pd.read_csv(
            '../data/testing_data_prep.csv', header=None)
    ...
    def predict(self, layer_weights, layer_bias):
        prediction_train = self.forwardProp(
        ...
        prediction_test = self.forwardProp(
    ...
    def forwardProp(self, features, layer_weights, layer_bias):
        ...
        for layer_weight, layer_bia in zip(layer_weights[:-1], layer_bias[:-1]):
            feat_list.append(self.activation_func_output(
                (feat_list[-1]).dot(layer_weight) + layer_bia))
        feat_list.append(
            (feat_list[-1]).dot(layer_weights[-1]) + layer_bias[-1])
        return feat_list

    def backwardProp(self, layer_weights, layer_bias, act_right, label):
        label_copy = label/100
    ...
    def curAccu(predicted, orig):
        ...
        for i in range(iter_shape):
            # print(f"predicted[i]: {predicted[i]} orig[i]: {orig[i]}")
