...
int main()
{
    ...
    // Get the sending buffer size
    if (getsockopt(sockfd, SOL_SOCKET, SO_SNDBUF, &sndbuf, &optlen) == -1)
    {
        perror("getsockopt for sending buffer size failed");
        close(sockfd);
        return 1;
    }

    // Get the receiving buffer size
    if (getsockopt(sockfd, SOL_SOCKET, SO_RCVBUF, &rcvbuf, &optlen) == -1)
    {
        perror("getsockopt for receiving buffer size failed");
        close(sockfd);
        return 1;
    }
