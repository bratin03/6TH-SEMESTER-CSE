...
Notes:
1. The serves is designed to receive emails for all domains. In case we want to receive emails for a particular domain, we can change the IP_ADDR macro to the domain name or for multiple domains, we can use a list of domains. If we want to receive emails for all domains, we can use a wildcard.
